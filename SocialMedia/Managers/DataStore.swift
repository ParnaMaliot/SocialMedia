...
class DataStore {
    ...
    func getFollowCount(userId: String, isFollowers: Bool, completion: @escaping (_ count: Int,_ error: Error?) -> Void) {
        //database/users/otherID or myId/following
        //database/users/otherID or myId/followers
    ...
    func getFollowings(completion: @escaping(_ following: [Following]?, _ error: Error?) -> Void) {
        ...
        followingRef.getDocuments { (snapshot, error) in
            ...
            if let snapshot = snapshot {
                do {
                    let followings = try snapshot.documents.compactMap({ try $0.data(as: Following.self) })
